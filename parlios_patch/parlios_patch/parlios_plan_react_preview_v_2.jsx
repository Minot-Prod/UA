// parlios_plan_react_preview_v_2.jsx.jsx — upgraded with UA/Base44
import React from "react";
import { useCreditLog, copyToClipboard, downloadBlob, printAsPDF, useOgCard, BlueprintPanel, CreditLog } from "./lib/ua";

export default function parlios_plan_react_preview_v_2() {
  const credit = useCreditLog();
  const [blueprint, setBlueprint] = React.useState({ goal:"", scope:"", acceptance:"", complexity:"" });
  const [input, setInput] = React.useState("");
  const [output, setOutput] = React.useState("# parlios_plan_react_preview_v_2.jsx\n\nPlanner Parlios (OKR allégés + tâches clés).\n");
  const refPdf = React.useRef(null);
  const og = useOgCard();

  React.useEffect(() => { credit.add("started", { app: "parlios_plan_react_preview_v_2.jsx" }) }, []);
  ...
  const compute = () => {
    ...
    og.draw("parlios_plan_react_preview_v_2.jsx".replace(".jsx",""), input.slice(0, 140));
  };

  const exportMd = () => { downloadBlob("parlios_plan_react_preview_v_2.md", output, "text/markdown"); credit.add("export_md"); };
  const exportPng = () => {
    const url = og.toDataURL();
    if (!url) return;
    fetch(url).then(r=>r.blob()).then((b) => downloadBlob("parlios_plan_react_preview_v_2.png", b, "image/png"));
  ...
  const share = async () => {
    ...
    try { if (navigator.share) await navigator.share({ title: "parlios_plan_react_preview_v_2.jsx", text: "Parlios — export", url: window.location.href }); } catch (e) {}
  };
    ...
    <div className="min-h-screen bg-[#0b0f14] text-white p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        <header className="flex items-center justify-between">
          <h1 className="text-2xl md:text-3xl font-bold">Parlios — parlios plan react preview v 2</h1>
