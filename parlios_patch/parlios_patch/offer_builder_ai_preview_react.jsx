// offer_builder_ai_preview_react.jsx.jsx — upgraded with UA/Base44
import React from "react";
import { useCreditLog, copyToClipboard, downloadBlob, printAsPDF, useOgCard, BlueprintPanel, CreditLog } from "./lib/ua";

export default function offer_builder_ai_preview_react() {
  const credit = useCreditLog();
  const [blueprint, setBlueprint] = React.useState({ goal:"", scope:"", acceptance:"", complexity:"" });
  const [input, setInput] = React.useState("");
  const [output, setOutput] = React.useState("# offer_builder_ai_preview_react.jsx\n\nWizard d’offre (segments, promesse, livrables, prix).\n");
  const refPdf = React.useRef(null);
  const og = useOgCard();

  React.useEffect(() => { credit.add("started", { app: "offer_builder_ai_preview_react.jsx" }) }, []);
  ...
  const compute = () => {
    ...
    og.draw("offer_builder_ai_preview_react.jsx".replace(".jsx",""), input.slice(0, 140));
  };

  const exportMd = () => { downloadBlob("offer_builder_ai_preview_react.md", output, "text/markdown"); credit.add("export_md"); };
  const exportPng = () => {
    const url = og.toDataURL();
    if (!url) return;
    fetch(url).then(r=>r.blob()).then((b) => downloadBlob("offer_builder_ai_preview_react.png", b, "image/png"));
  ...
  const share = async () => {
    ...
    try { if (navigator.share) await navigator.share({ title: "offer_builder_ai_preview_react.jsx", text: "Parlios — export", url: window.location.href }); } catch (e) {}
  };
    ...
    <div className="min-h-screen bg-[#0b0f14] text-white p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        <header className="flex items-center justify-between">
          <h1 className="text-2xl md:text-3xl font-bold">Parlios — offer builder ai preview react</h1>
