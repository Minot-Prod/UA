...
export default function handler(req: NextApiRequest, res: NextApiResponse){
  ...
  const { email, password, csrf, role } = req.body || {}
  ...
  // Rôle en cookie lisible par middleware (démo). En prod: signe et stocke côté serveur.
  res.setHeader('Set-Cookie', `parlios.role=${safeRole}; Path=/; Max-Age=${60*60*24*7}; SameSite=Lax; Secure`)
