  ...
  const loadData = async () => {
    try {
      const [coursesData, lessonsData, progressData, userData] = await Promise.all([
        Course.filter({ language: "python" }, "order"),
        Lesson.filter({ language: "python" }, "order"), // Changed from Lesson.list
        UserProgress.list(),
        User.me().catch(() => null)
      ]);
  ...
  const markLessonComplete = async (lessonId) => {
    ...
    try {
      await UserProgress.create({
        ...
        course_slug: selectedLesson?.course_slug, // Changed from course_id
        lesson_id: lessonId,
        completed: true,
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl font-bold text-gray-900">Python Programming</h1>
            </div>
            <div className="flex items-center gap-4">
              <Progress value={getProgressPercentage()} className="flex-1 max-w-xs h-2 [&>div]:bg-blue-500" />
                ...
                <div className="space-y-1">
                  {lessons.map((lesson, index) => (
                    <button
                      ...
                      className={`w-full text-left p-4 hover:bg-blue-50 transition-colors border-l-4 rounded-r-lg ${
              ...
              <>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <Button
                    ...
                    className="bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 text-white font-semibold px-6 py-3 rounded-xl shadow-lg"
                  >
              ...
              <Card className="border-0 shadow-lg bg-white/90 backdrop-blur-sm">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-gray-600">
                    Python lessons will be added soon. Check back later!
                  </p>
                </CardContent>
              </Card>
            )}
